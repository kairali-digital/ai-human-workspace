...
**DRIVE-HW-001** - build the complete connector-visible Google Drive metadata master
index in checkpointed batches. `TEST 25` may prove setup, but only `FULL DRIVE INDEX`
completes company homework and unlocks the optional weekly-refresh offer.

## NEXT ACTION

Personalize the human owner, confirm the Google Drive app is connected to the
approved company account, then visibly ask the owner to choose `TEST 25` or `FULL
DRIVE INDEX`.
