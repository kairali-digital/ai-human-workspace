# Start here — Weekly LinkedIn Message Assistant (optional)
...
1. Complete Email Triage and Drive Inventory first.
2. Open this folder as a third, separate local project.
3. Choose Codex and start a new chat inside this project.
4. Paste the first-run prompt below after replacing `[TYPE YOUR NAME]`.
5. Answer or perform only the one action shown. Wait for **DONE WHEN** before continuing.
6. Keep all copied message text in this local project. Never put it in the shared company
   repository.

## Paste this exact first-run prompt

```text
My name is [TYPE YOUR NAME]. I choose the optional task LINKEDIN-WEEKLY-001.
...
1. Ask exactly: “What local time every Saturday should your LinkedIn message review
...
When the pilot passes, update AUTOMATIONS.md, the ledger, register, cursor, today file,
and evidence log. Then validate this workspace and show the schedule, pilot counts,
remaining queue count, and the exact manual-send boundary.
```
