...
Your mission is to close your Marketing rows in the Master Delegation Sheet. You choose the row and judge the result. Codex sorts, prepares, executes approved repeat work and records proof. Website and domain counts are not your responsibility.
...
**Open rows on your name today:** 318
...
Read them. Do not suggest anything yet. Just tell me how many there are
and how many are still pending on my name.
```
...
```
Sort my rows into three groups:
A - the AI can do this for me now
B - I have to do it, but the AI can prepare it
C - only I can do it
One line on why, for each row. Do nothing else yet.
```

## 2. Find the repeat work

```
Look at group A. Which of these are the same job over and over?
Group them. Those are the ones we automate first.
```

## 3. Run one batch

```
Take 25 rows from the biggest group. Do them one at a time.
After each one write: what was done, and the proof.
Stop at 25.
```
...
```
List every row you could not finish, with the reason in one line.
```
---
