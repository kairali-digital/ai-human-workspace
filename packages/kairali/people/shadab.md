...
**Where it is:** https://docs.google.com/spreadsheets/d/1RLslLN9Mc-npXmX1_A82dzDlSaJyAA4nEnz2zKo_fG8/edit
**Open rows on your name today:** 42 on the delegation sheet, plus the 51 above
...
```
Here are my 51 Shopify tasks.
Sort them by priority, and group the ones that can be done in one pass.
Give me the first 25 that can run together.
```

## 2. Before you change anything

```
For this task: show me the current state first — a readback or a screenshot.
Then tell me the smallest change that fixes it, and how to undo it.
```

## 3. Make the change

```
Make that change. Then show me the readback proving it is live.
Not the setting. The live result.
```

## 4. Close it

```
Write the ledger row: what changed, the proof, what is still limited,
and what would make us reopen this.
```
---
