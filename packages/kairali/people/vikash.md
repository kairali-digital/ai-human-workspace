...
Your mission is one measurement-audit task for your division. You judge the business impact. Codex audits, records defects, fixes only the approved batch and proves the result before you compare it with Rohan.
...
**Open rows on your name today:** 124
...
```
Check the measurement setup on my division:
GA4, tags, events, conversions, and anything missing or double-counting.
Use the dual-store audit as the template.
Give me a list of defects. Fix nothing.
```
...
Each row: ID, what is wrong, where, how bad, what proves it.
```

## 3. Fix the top 25 only

```
Fix the 25 highest-impact defects. No more.
For each one: before, after, how to undo it.
```
---
